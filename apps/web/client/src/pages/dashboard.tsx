...
import {
  MemoryAssessmentBanner,
  useMemoryAssessmentBanner,
} from "@/components/memory-assessment-banner";
import MemoryAssessmentModal from "@/components/memory-assessment-modal";
...
export default function Dashboard() {
  ...
  // Memory assessment banner logic
  const {
    showBanner,
    showAssessmentModal,
    onStartAssessment,
          ...
          <div className="animate-pulse">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="bg-card p-6 rounded-xl border border-border h-24"
                ></div>
              ))}
            </div>
          </div>
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Memory Assessment Banner - shown at top for users who haven't completed assessment */}
      {showBanner && (
        <MemoryAssessmentBanner
